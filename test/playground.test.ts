...
describe('Parse Sample codes', () => {
	...
	it('generate PlantUML for Abstract/AbstractClass.ts', () => {
		expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Abstract/AbstractClass.ts'])))
		.toEqual(
			['@startuml',
		...
		it('generate PlantUML for Classes/Greeter.ts', () => {
			expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Classes/Greeter.ts'])))
			.toEqual(
				['@startuml',
				'class Greeter {',
				'    +greeting: string',
			...
			it('generate PlantUML for Enum/Enum.ts', () => {
				expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Enum/Enum.ts'])))
				.toEqual(
					['@startuml',
					'enum Semaphore {',
					'    RED',
				...
				it('generate PlantUML for Inheritance', () => {
					expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Inheritance/index.ts'])))
					.toEqual(
						['@startuml',
						'class Animal {',
						'    +name: string',
					...
					it('generate PlantUML for Generics/Complex.ts', () => {
						expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Generics/Complex.ts'])))
						.toEqual(
							['@startuml',
						...
						it('generate PlantUML for Generics/Greeter.ts', () => {
							expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Generics/Greeter.ts'])))
							.toEqual(
								['@startuml',
								'class Greeter<T> {',
								'    +greeting: T',
							...
							it('generate PlantUML for Generics/RecursiveGenericType.ts', () => {
								expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Generics/RecursiveGenericType.ts'])))
								.toEqual(
									['@startuml',
								...
								it('generate PlantUML for Inheritance/autos.ts', () => {
									expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/Inheritance/autos.ts'])))
									.toEqual(
										['@startuml',
									...
									it('generate PlantUML for RayTracer', () => {
										expect(tplant.convertToPlant(tplant.generateDocumentation(['sample/Playground/RayTracer/index.ts'])))
										.toEqual(
											['@startuml',
											'class Vector {',
											'    +x: number',
